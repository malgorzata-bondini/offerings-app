...
def build_standard_name(parent_offering, sr_or_im, app, schedule_suffix, special_dept=None):
    ...
    if special_dept == "Medical":
       ...
       prefix_parts = [sr_or_im]
       # For IT with UA/MD, always use DS
       if country in ["UA", "MD"]:
...
def run_generator(*,
   ...
   for wb in src_dir.glob("ALL_Service_Offering_*.xlsx"):
       ...
       # Ensure Visibility group column exists but will be removed for PL
       ...
       if require_corp or require_corp_it or require_corp_dedicated:
           mask &= df["Name (Child Service Offering lvl 1)"].str.contains(r"\bCORP\b",case=False)
           if not require_corp_it and not require_corp_dedicated:
       ...
       for idx, base_row in base_pool.iterrows():
           ...
           if require_corp or require_recp or require_corp_it or require_corp_dedicated:
               ...
               # For standard offerings, also create double entries for DE and CY
               if country == "DE":
                   receivers = ["DS DE", "HS DE"]
               elif country == "CY":
                   receivers = ["DS CY", "HS CY"]
           ...
           for app in all_apps:
               for recv in receivers:
                   for schedule_suffix in schedule_suffixes:
                       ...
                       # Preserve columns from original row
                       for col in preserve_cols:
                           if col in base_row_df.columns:
                               row[col] = base_row_df.iloc[0][col]
                       
                       # Handle aliases specially
                       if not aliases_on:
                           # Copy aliases from original row
                           alias_cols = [c for c in base_row_df.columns if "Aliases" in c]
                           for c in alias_cols:
                               if c in base_row_df.columns:
                                   row[c] = base_row_df.iloc[0][c]
                       else:
                           # Use provided alias value
                       ...
                       if global_prod:
                           ...
                           # Handle IT special case for UA/MD
                           if special_dept == "IT" and country in ["UA", "MD"] and not global_prod:
